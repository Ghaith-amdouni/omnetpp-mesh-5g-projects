//
// Generated file, do not edit! Created by opp_msgtool 6.3 from inet/routing/pim/PimPacket.msg.
//
...
std::string HelloOptionDescriptor::getFieldValueAsString(omnetpp::any_ptr object, int field, int i) const
{
    ...
    HelloOption *pp = omnetpp::fromAnyPtr<HelloOption>(object); (void)pp;
    switch (field) {
        case FIELD_type: return enum2string(static_cast<int>(pp->getType()), "inet::PimHelloOptionType");
        default: return "";
    }
}
...
std::string PimPacketDescriptor::getFieldValueAsString(omnetpp::any_ptr object, int field, int i) const
{
    ...
    switch (field) {
        ...
        case FIELD_type: return enum2string(static_cast<int>(pp->getType()), "inet::PimPacketType");
        case FIELD_reserved: return long2string(pp->getReserved());
        case FIELD_crc: return ulong2string(pp->getCrc());
        case FIELD_crcMode: return enum2string(static_cast<int>(pp->getCrcMode()), "inet::CrcMode");
        default: return "";
    }
}
...
void PimHello::setOptions(size_t k, HelloOption * options)
{
    ...
    if (this->options[k] != nullptr && this->options[k]->isOwnedObject()) dropAndDelete((omnetpp::cOwnedObject*)this->options[k]); else delete this->options[k];
    this->options[k] = options;
    if (this->options[k] != nullptr && this->options[k]->isOwnedObject()) take((omnetpp::cOwnedObject*)this->options[k]);
}
...
void PimHello::insertOptions(size_t k, HelloOption * options)
{
    ...
    for (i = 0; i < k; i++)
        options2[i] = this->options[i];
    options2[k] = options;
    if (options2[k] != nullptr && options2[k]->isOwnedObject()) take((omnetpp::cOwnedObject*)options2[k]);
    for (i = k + 1; i < newSize; i++)
