//
// Generated file, do not edit! Created by opp_msgtool 6.3 from inet/networklayer/icmpv6/Ipv6NdMessage.msg.
//
...
std::string Ipv6NdOptionDescriptor::getFieldValueAsString(omnetpp::any_ptr object, int field, int i) const
{
    ...
    Ipv6NdOption *pp = omnetpp::fromAnyPtr<Ipv6NdOption>(object); (void)pp;
    switch (field) {
        case FIELD_type: return enum2string(static_cast<int>(pp->getType()), "inet::Ipv6NdOptionTypes");
...
void Ipv6NdOptions::setOption(size_t k, Ipv6NdOption * option)
{
    ...
    if (this->option[k] != nullptr && this->option[k]->isOwnedObject()) take((omnetpp::cOwnedObject*)this->option[k]);
}
...
void Ipv6NdOptions::insertOption(size_t k, Ipv6NdOption * option)
{
    ...
    for (i = 0; i < k; i++)
        option2[i] = this->option[i];
    option2[k] = option;
    if (option2[k] != nullptr && option2[k]->isOwnedObject()) take((omnetpp::cOwnedObject*)option2[k]);
    for (i = k + 1; i < newSize; i++)
...
void Ipv6NdOptions::eraseOption(size_t k)
{
    ...
    for (i = k; i < newSize; i++)
        option2[i] = this->option[i+1];
    if (this->option[k] != nullptr && this->option[k]->isOwnedObject()) dropAndDelete((omnetpp::cOwnedObject*)this->option[k]); else delete this->option[k];
    delete [] this->option;
    this->option = option2;
    option_arraysize = newSize;
}
