//
// Generated file, do not edit! Created by opp_msgtool 6.3 from inet/common/TlvOptions.msg.
//
...
void TlvOptions::setTlvOption(size_t k, TlvOptionBase * tlvOption)
{
    ...
    if (this->tlvOption[k] != nullptr && this->tlvOption[k]->isOwnedObject()) take((omnetpp::cOwnedObject*)this->tlvOption[k]);
}
...
void TlvOptions::insertTlvOption(size_t k, TlvOptionBase * tlvOption)
{
    ...
    if (tlvOption2[k] != nullptr && tlvOption2[k]->isOwnedObject()) take((omnetpp::cOwnedObject*)tlvOption2[k]);
    for (i = k + 1; i < newSize; i++)
...
void TlvOptions::eraseTlvOption(size_t k)
{
    ...
    for (i = k; i < newSize; i++)
        tlvOption2[i] = this->tlvOption[i+1];
    if (this->tlvOption[k] != nullptr && this->tlvOption[k]->isOwnedObject()) dropAndDelete((omnetpp::cOwnedObject*)this->tlvOption[k]); else delete this->tlvOption[k];
    delete [] this->tlvOption;
    this->tlvOption = tlvOption2;
    tlvOption_arraysize = newSize;
}
