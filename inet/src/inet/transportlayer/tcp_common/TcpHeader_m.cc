//
// Generated file, do not edit! Created by opp_msgtool 6.3 from inet/transportlayer/tcp_common/TcpHeader.msg.
//
...
std::string TcpOptionDescriptor::getFieldValueAsString(omnetpp::any_ptr object, int field, int i) const
{
    ...
    TcpOption *pp = omnetpp::fromAnyPtr<TcpOption>(object); (void)pp;
    switch (field) {
        case FIELD_kind: return enum2string(static_cast<int>(pp->getKind()), "inet::tcp::TcpOptionNumbers");
...
void TcpHeader::setHeaderOption(size_t k, TcpOption * headerOption)
{
    ...
    if (this->headerOption[k] != nullptr && this->headerOption[k]->isOwnedObject()) take((omnetpp::cOwnedObject*)this->headerOption[k]);
}
...
void TcpHeader::insertHeaderOption(size_t k, TcpOption * headerOption)
{
    ...
    if (headerOption2[k] != nullptr && headerOption2[k]->isOwnedObject()) take((omnetpp::cOwnedObject*)headerOption2[k]);
    for (i = k + 1; i < newSize; i++)
...
void TcpHeader::eraseHeaderOption(size_t k)
{
    ...
    for (i = k; i < newSize; i++)
        headerOption2[i] = this->headerOption[i+1];
    if (this->headerOption[k] != nullptr && this->headerOption[k]->isOwnedObject()) dropAndDelete((omnetpp::cOwnedObject*)this->headerOption[k]); else delete this->headerOption[k];
    delete [] this->headerOption;
    this->headerOption = headerOption2;
    headerOption_arraysize = newSize;
}
